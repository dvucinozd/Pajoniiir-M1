...
`11_TOUCH_GT911` and `15_DNP_OPTIONS` intentionally contain no instantiated symbols after migration to the generic DSI host. Their presence preserves the hierarchy and historical sheet roles.
...
| Generic 15-pin DSI host contract | PASS |
| Complete hierarchy load under KiCad 10.0.4 | PASS (16-sheet ERC/PDF export) |
| Local native KiCad 10.0.4 ERC | 0 unexplained errors, 0 excluded errors, 0 warnings |
| Manufacturing BOM parity | 244 source / 244 export, 15 DNP, 3 blank gates, PASS |
| Final PCB placement/routing freeze | BLOCKED |

The `.kicad_pro` file still contains six UUID-scoped exclusion records from the retired JC4880 connector gate. The current native KiCad 10 ERC report contains no violations, so those records are inactive configuration history rather than current violations.

See [PG diagnostics and library closure](Pajoniiir_M1_ERC_Library_Closure_2026-09-04.md) for TP25/TP26 connectivity, connector shield preservation, U14 land pattern and the remaining export-warning observation.
...
The active design is a generic 15-pin Raspberry-Pi-style DSI host. J6 is Amphenol SFW15R-2STE1LF with a locked project footprint; DSI506/DYL0023 is one validated profile.
...
The remaining J6 gate covers board-edge placement, power budget, generic service clearance and cable/adapter qualification per display model.
...
- exact project eFuse and generic DSI host connector contracts
- `5V_PROTECTED -> R120 -> 5V_SYS` topology
- blank-footprint allowlist derived from `mechanical_gates.json`
- B7 board-first authority and fail-closed mechanical/PCB state
