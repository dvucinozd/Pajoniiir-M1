# Pajoniiir Mainboard — MIPI DSI Display & Backlight Design v0.1
...
# 16. Why backlight values are not locked yet

Za točan MP3202 calculation trebamo znati:

- broj LED-ova u seriji
- broj paralelnih stringova
- Vf distribution
- nominalni total LED current
- max LED current
- LEDA/LEDK arrangement

Točni parametri nisu dovoljno pouzdano potvrđeni iz javne dokumentacije.

Zato se još ne zaključavaju:
...
Do authoritative panel/backlight speca ove stavke ostaju:

**TBD-PANEL**

---

# 17. Important PWM finding

Aktualni Pajoniiir JC4880 BSP koristi:

**GPIO23 LEDC PWM @ 5 kHz**

MP3202 datasheet za direct PWM na EN preporučuje približno:

**1 kHz ili niže**

zbog soft-start ponašanja.

Za PWM iznad 1 kHz MPS pokazuje alternativni filtered-PWM feedback control.

To znači da na custom PCB-u ne smijemo samo pretpostaviti:

~~~text
GPIO23 5kHz -> MP3202 EN
~~~
...
# 18. Rev A backlight control decision

Za prvu ploču preferiram **zadržati postojeći 5 kHz firmware** i koristiti MP3202 high-frequency dimming pristup preko filtered control patha, umjesto cikličkog gašenja EN-a na 5 kHz.

Arhitekturno:

~~~text
MP3202 EN
 -> hardware enabled when display rail valid

GPIO23 5kHz PWM
 -> RC filter / injection network
 -> MP3202 FB dimming control
~~~

Točnu R/C injection mrežu zaključati istovremeno s finalnim R_BL_SENSE nakon što znamo LED current.

Prednosti:

- postojeći brightness API ostaje
- nema soft-starta 5000 puta/s
- nema potrebe odmah mijenjati UI/brightness firmware behavior
- smooth dimming
...
Iako brightness PWM ide kroz FB dimming, želimo moći potpuno ugasiti driver.
...
1. GPIO23 preko diode/logic combination i filtered PWM
2. poseban spare GPIO
3. hardware EN tied high only after 5V_SYS

Za Rev A najjednostavnije:

- MP3202 EN ima 100 kΩ pulldown
- 3V3 pull-up/enable strap kroz 0 Ω
- service DNP path za GPIO control

Full display power shutdown nije trenutno product requirement, pa ga ne treba nepotrebno komplicirati.

---

# 20. Backlight EMI placement

MP3202 je visokofrekventni boost i mora biti fizički udaljen od:
...
- minimalna površina
- što kraća petlja
- bez copper flooda koji nepotrebno povećava capacitance
- ne stavljati pod MIPI pair
...
MP3202 open-load/OV node mora biti vezan prema datasheet reference circuitu nakon finalnog LED string calculationa.
...
| L_BL | 1 | TBD | final panel dependent |
| D_BL | 1 | TBD | final boost calculation |
| R_BL_SENSE | 1 | TBD | final LED current |
| C_BL_IN | set | TBD | after final calculation |
| C_BL_OUT | set | TBD | after final calculation |
| R/C_BL_PWM | set | TBD | filtered 5kHz dimming network |
...
- [ ] panel 3.3 V current
- [ ] LED string Vf/current specification
- [ ] MP3202 inductor/diode/current-sense calculation
- [ ] 5 kHz filtered dimming network calculation
...
Jedini namjerno nezaključani dijelovi su oni koji stvarno ovise o exact panelu: FPC footprint i backlight power component values.
