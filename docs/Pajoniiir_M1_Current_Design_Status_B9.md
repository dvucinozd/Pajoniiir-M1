...
**Pre-layout milestone:** M1-PRELAYOUT-B9

**Layout state:** populated board-first canvas with a compact P4 routing-feasibility island; `layout_freeze_allowed=false`
...
M1-PRELAYOUT-B8 populated the live PCB with all 244 assigned footprints and 193 named schematic nets. Eight `Dwgs.User` rectangles separate the working electrical domains: power spine, USB edges, audio, P4 core, C6 RF, DSI host, storage and service.

M1-PRELAYOUT-B9 moves all 63 footprints from `03_P4_CORE` and `04_P4_FLASH_CLOCK_RESET` into a deterministic 52.602 x 50.033 mm routing-feasibility island. U1, U2, U3, L1 and Y1 now form the electrical anchors; the six QSPI tuning resistors sit between U1 and U2, the external DCDC loop sits beside U1, and the 40 MHz crystal body remains 7.285 mm from U1. The island has zero footprint bounding-box overlaps and the entire board still has no tracks, zones or `Edge.Cuts`.

`m1_prelayout_b9_core_island.json` records every B9 coordinate and metric. These coordinates and the eight domain rectangles remain a reversible working canvas. They are not connector, mounting or enclosure datums, and the next escape/routing pass may move parts within the P4 island.
...
- the reproducible B9 P4 core-island seed and its fail-closed validator;
...
1. Prove the U1 escape corridors and short QSPI, external-DCDC and crystal routes inside the B9 island while retaining a complete GND reference.
