...
> The JC4880/Guition geometry below is retained only as historical enclosure evidence. DSI506/DYL0023 is also a display compatibility profile rather than mainboard geometry authority. Do not derive `Edge.Cuts`, board mounts or Z limits from either screen. Current authority: `board_first_mechanical_contract.json`, `mechanical_gates.json` and `pcb_constraints.json`.
